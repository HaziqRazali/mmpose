    ...
    if depth_img is not None:
        ...
        if intrin_dict is not None:
            ...
            if keypoints.shape[0] > 0:
                ...
                joints_xyz = np.array(joints_xyz) # [N, J, 3]

                # accumulate angle
                kpt_ids = rom_test[args.rom_test]
                angle_t = angle(joints_xyz[0,kpt_ids[0]], joints_xyz[0,kpt_ids[1]], joints_xyz[0,kpt_ids[2]])
                
                # 3D visualization
                if getattr(args, "show3d", False):
                    visualize_3d_skeletons(
                        joint_xyz, skeleton, args.show_kpt_subset, args.kpt_thr
                    )
